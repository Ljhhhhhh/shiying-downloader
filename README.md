...
2. 粘贴视频链接，或直接粘贴含链接的小红书、抖音分享文案；选择最佳画质、最高 1080p、最高 720p 或 M4A 音频。
3. 点击「开始下载」。默认保存至系统下载文件夹内的「拾影」，也可以更改位置。

分享文案会取第一个有效链接；短链接由下载工具跟随跳转。一次下载一个视频，不展开播放列表。分离的音视频自动合并为 MKV；已有完整音视频文件保留源格式。取消后可以重试，服务器支持续传时会利用未完成文件。清空记录不会删除已下载文件。

网站可用性取决于网络、站点规则和 yt-dlp 支持情况。本版本不含账号登录、Cookie 导入、付费或 DRM 内容解密。需要登录或受访问限制的视频可能无法下载。关闭应用会中断当前任务，下次打开可重试。

## 抖音视频（macOS + Chrome 本地试用）

1. 在 `chrome://extensions/` 打开开发者模式，点「加载已解压的扩展程序」，选择仓库中的 `extension` 文件夹。更新扩展源码后，点扩展卡片上的「重新加载」。
2. 打开单条抖音视频详情页并播放几秒，点击 Chrome 工具栏中的「发送到拾影」。
3. 在拾影确认标题和当前播放画质，点击「开始下载」。画面与声音会合并成一个 MP4 文件。

如果提示找不到同一视频的画面和声音地址，请刷新视频页、重新播放，再发送。地址失效后也按此步骤重新取址；历史记录中的「重新发送」不会复用旧地址。扩展只读取当前标签页，不读取 Cookie。
